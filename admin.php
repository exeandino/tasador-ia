...
<div class="panel" id="tab-importar">
  <div class="topbar"><h2>📥 Importar propiedades</h2></div>

  <!-- Sub-tabs CSV / XML -->
  <div style="display:flex;gap:4px;margin-bottom:18px">
    <button id="itab-csv" onclick="showImportTab('csv')"
      style="padding:8px 20px;border-radius:8px 8px 0 0;font-size:13px;font-weight:600;cursor:pointer;border:1px solid var(--border);border-bottom:none;background:var(--card);color:var(--gold);font-family:var(--font)">
      📊 CSV / Excel
    </button>
    <button id="itab-xml" onclick="showImportTab('xml')"
      style="padding:8px 20px;border-radius:8px 8px 0 0;font-size:13px;font-weight:600;cursor:pointer;border:1px solid var(--border);border-bottom:none;background:var(--bg2);color:var(--muted);font-family:var(--font)">
      🗂 WordPress XML
    </button>
  </div>
  ...
  <div id="ipanel-csv">
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:16px">
        <div>
          <div style="font-size:14px;font-weight:700;margin-bottom:3px">Importar desde Excel / CSV</div>
          <div style="font-size:12px;color:var(--muted)">Completá la plantilla en Excel y exportá como CSV para subir</div>
        </div>
        <a href="propiedades-ejemplo.xlsx" download="propiedades-ejemplo.xlsx"
           style="display:inline-flex;align-items:center;gap:7px;padding:8px 18px;background:rgba(76,175,80,.12);border:1px solid rgba(76,175,80,.4);border-radius:8px;color:#4caf50;font-size:12px;font-weight:700;text-decoration:none">
          ⬇ Descargar plantilla Excel
        </a>
      </div>
      <div style="background:rgba(74,143,247,.07);border:1px solid rgba(74,143,247,.2);border-radius:8px;padding:12px 16px;margin-bottom:18px;font-size:12px;color:#a0b4d0;line-height:1.9">
        <strong style="color:var(--blue)">Cómo usarlo:</strong>
        1 · Descargá la plantilla Excel &nbsp;→&nbsp;
        2 · Completá una fila por propiedad (las amarillas son obligatorias) &nbsp;→&nbsp;
        3 · <strong>Archivo → Guardar como → CSV (separado por comas)</strong> &nbsp;→&nbsp;
        4 · Subí el CSV acá abajo
      </div>
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:18px;text-align:center">
        <div style="padding:12px;background:var(--bg2);border-radius:8px;font-size:13px"><strong style="color:#4caf50;font-size:20px">1</strong><br>Descargá y completá la plantilla Excel</div>
        <div style="padding:12px;background:var(--bg2);border-radius:8px;font-size:13px"><strong style="color:#4caf50;font-size:20px">2</strong><br>Guardá como CSV desde Excel</div>
        <div style="padding:12px;background:var(--bg2);border-radius:8px;font-size:13px"><strong style="color:#4caf50;font-size:20px">3</strong><br>Subí el CSV y hacé clic en Importar</div>
      </div>
      <div class="field" style="margin-bottom:12px">
        <label style="font-size:12px;color:var(--muted);text-transform:uppercase;letter-spacing:.5px">Archivo CSV</label>
        <div class="drop-zone" id="cdz" ondragover="this.classList.add('dragging');event.preventDefault()" ondragleave="this.classList.remove('dragging')" ondrop="this.classList.remove('dragging')">
          <input type="file" id="csv-file-input" accept=".csv" onchange="showCsvFn(this)">
          <div style="font-size:28px;margin-bottom:6px">📊</div>
          <div style="font-size:13px;color:var(--muted)">Arrastrá el CSV o hacé clic</div>
          <div id="cfn" style="font-size:12px;color:var(--gold);margin-top:4px"></div>
        </div>
      </div>
      <button class="btn btn-gold" id="csv-import-btn" onclick="importCsv()" disabled>
        ✦ Importar CSV
      </button>
      <div id="csv-result" style="display:none;margin-top:14px;padding:12px 16px;border-radius:8px;font-size:13px"></div>
    </div>
  </div>

  <!-- ── Panel XML ── -->
  <div id="ipanel-xml" style="display:none">
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;margin-bottom:16px">
        <div>
          <div style="font-size:14px;font-weight:700;margin-bottom:3px">Importar desde WordPress / Houzez XML</div>
          <div style="font-size:12px;color:var(--muted)">Export WXR de WordPress con tema Houzez</div>
        </div>
        <a href="houzez-ejemplo.xml" download="houzez-ejemplo.xml"
           style="display:inline-flex;align-items:center;gap:7px;padding:8px 18px;background:rgba(201,168,76,.12);border:1px solid rgba(201,168,76,.35);border-radius:8px;color:var(--gold);font-size:12px;font-weight:700;text-decoration:none">
...
function showImportTab(tab) {
  const isCSV = tab === 'csv';
  document.getElementById('ipanel-csv').style.display = isCSV ? '' : 'none';
  document.getElementById('ipanel-xml').style.display = isCSV ? 'none' : '';
  const activeStyle  = 'padding:8px 20px;border-radius:8px 8px 0 0;font-size:13px;font-weight:600;cursor:pointer;border:1px solid var(--border);border-bottom:none;background:var(--card);font-family:var(--font);';
  const inactiveStyle = 'padding:8px 20px;border-radius:8px 8px 0 0;font-size:13px;font-weight:600;cursor:pointer;border:1px solid var(--border);border-bottom:none;background:var(--bg2);color:var(--muted);font-family:var(--font);';
  document.getElementById('itab-csv').style.cssText = isCSV ? activeStyle + 'color:var(--gold)' : inactiveStyle;
  document.getElementById('itab-xml').style.cssText = isCSV ? inactiveStyle : activeStyle + 'color:var(--gold)';
}

// ── CSV import ────────────────────────────────────────────────
function showCsvFn(inp) {
  const fn = inp.files[0]?.name;
  document.getElementById('cfn').textContent = fn ? '✓ ' + fn : '';
  document.getElementById('csv-import-btn').disabled = !fn;
}
...
async function importCsv() {
  ...
  try {
    ...
    const j = await (await fetch('api/import_csv.php', {method:'POST', body:fd})).json();
    res.style.display = '';
    if (j.success) {
      res.style.background = 'rgba(76,175,80,.1)';
      res.style.border = '1px solid rgba(76,175,80,.3)';
      res.style.color = 'var(--green)';
      res.innerHTML = '✓ ' + j.msg +
        (j.errors?.length ? '<br><small style="color:var(--red)">' + j.errors.join('<br>') + '</small>' : '');
      btn.textContent = '✓ Importado';
      btn.style.background = 'var(--green)';
    } else {
      res.style.background = 'rgba(244,67,54,.1)';
      res.style.border = '1px solid rgba(244,67,54,.3)';
      res.style.color = 'var(--red)';
      res.textContent = '✗ ' + j.error;
      btn.disabled = false;
