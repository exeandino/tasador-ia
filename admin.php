...
<!-- ── PESTAÑA: BUSCADOR ─────────────────────────────────────────────────────
     Agregar este bloque en admin.php como nueva pestaña del sidebar y panel.
     
     1. En el sidebar agregar:
        <button class="nav-item" onclick="showTab('buscador',this)"><span class="icon">🔍</span>Buscador</button>
     
     2. Agregar este panel dentro del <div class="main">
──────────────────────────────────────────────────────────────────────────── -->
...
<div class="panel" id="tab-buscador">
  ...
  <div class="card" style="margin-bottom:14px">
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:12px">
      ...
      <div class="field">
        ...
        <input type="text" id="sf-texto" placeholder="dirección, zona, título..." onkeyup="if(event.key==='Enter')doSearch()">
      </div>
      <div class="field">
        <label>Ciudad</label>
        <input type="text" id="sf-ciudad" placeholder="Santa Fe, Buenos Aires...">
      </div>
      <div class="field">
        <label>Zona / Barrio</label>
        <input type="text" id="sf-zona" placeholder="Candioti, Palermo...">
      </div>
      <div class="field">
        <label>Tipo</label>
        <select id="sf-tipo">
      ...
      <div class="field">
        ...
        <input type="number" id="sf-pmin" placeholder="0" min="0" step="1000">
      </div>
      <div class="field">
        <label>Precio USD máx</label>
        <input type="number" id="sf-pmax" placeholder="999999" min="0" step="1000">
      </div>
      <div class="field">
        <label>Superficie m² mín</label>
        <input type="number" id="sf-amin" placeholder="0" min="0" step="5">
      </div>

      <div class="field">
        <label>Superficie m² máx</label>
        <input type="number" id="sf-amax" placeholder="999" min="0" step="5">
      </div>
      <div class="field">
        <label>Dormitorios mín</label>
        <input type="number" id="sf-dmin" placeholder="0" min="0" max="10">
      </div>
      <div class="field">
        <label>Baños mín</label>
        <input type="number" id="sf-banos" placeholder="0" min="0" max="5">
      </div>
      <div class="field">
        <label>Cochera</label>
        <select id="sf-cochera">
    ...
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
      ...
      <div style="display:flex;align-items:center;gap:8px;margin-left:auto">
        ...
        <select id="sf-order" onchange="doSearch()" style="padding:6px 10px;background:var(--bg3);border:1px solid var(--border);border-radius:6px;color:var(--text);font-size:12px">
          <option value="scraped_at">Fecha</option>
          <option value="price_usd">Precio</option>
          <option value="price_per_m2">USD/m²</option>
          <option value="covered_area">Superficie</option>
        </select>
  ...
  <div id="search-results-wrap" style="display:none">
    <table class="tbl" id="search-table">
      <thead>
        <tr>
          <th>Propiedad</th>
          <th>Tipo</th>
...
function doSearch(resetPage) {
    ...
    fetch('api/search_properties.php', {
        method:  'POST',
    ...
    .catch(e => {
        ...
        document.getElementById('search-error').textContent     = 'Error: ' + e.message;
    });
}
...
function renderResults(d) {
    ...
    if (d.error) {
        document.getElementById('search-error').style.display = 'block';
        document.getElementById('search-error').textContent   = 'Error: ' + d.error;
        return;
    }
    ...
    if (d.stats) {
        ...
        document.getElementById('ss-range').textContent = d.stats.min_precio && d.stats.max_precio
            ? 'USD ' + fmt(d.stats.min_precio) + ' — ' + fmt(d.stats.max_precio) : '—';
    }

    document.getElementById('search-total-info').textContent =
        d.total + ' resultado' + (d.total!==1?'s':'') + ' · mostrando ' + d.showing;

    if (d.total === 0) {
        document.getElementById('search-empty').style.display = 'block'; return;
    }
    ...
    var rows = d.results.map(function(r) {
        var titleShort = (r.address || r.title || '').slice(0,40);
        var titleFull  = r.address || r.title || '—';
        var urlLink    = r.url
            ? '<a href="' + r.url + '" target="_blank" style="color:var(--gold)" title="' + titleFull + '">' + titleShort + ' ↗</a>'
            : '<span title="' + titleFull + '">' + titleShort + '</span>';

        var specs = [];
        if (r.bedrooms)  specs.push(r.bedrooms + ' dorm');
        if (r.bathrooms) specs.push(r.bathrooms + 'b');
        if (r.garages)   specs.push('🚗');

        return '<tr>'
            + '<td>' + urlLink + (r.zone ? '<br><small style="color:var(--muted)">' + r.zone + '</small>' : '') + '</td>'
            + '<td><span style="color:var(--muted);font-size:12px">' + (r.property_type||'—') + '</span></td>'
            + '<td>' + (r.covered_area ? r.covered_area + 'm²' : '—') + '</td>'
            + '<td style="font-size:12px">' + specs.join(' · ') + '</td>'
            ...
            + '<td style="font-size:11px;color:var(--muted)">' + (r.scraped_at||'—') + '</td>'
            + '</tr>';
    }).join('');
    ...
    var currentPage = searchPage + 1;
    document.getElementById('page-info').textContent = 'Pág. ' + currentPage + ' de ' + totalPages + ' (' + d.total + ' total)';
...
function changePage(dir) {
    ...
    doSearch(false);
    document.getElementById('tab-buscador').scrollIntoView({behavior:'smooth'});
}

function clearSearch() {
    ['sf-texto','sf-ciudad','sf-zona','sf-pmin','sf-pmax','sf-amin','sf-amax','sf-dmin','sf-banos'].forEach(id => {
        var el = document.getElementById(id); if (el) el.value = '';
    });
    ['sf-tipo','sf-op','sf-cochera'].forEach(id => {
        var el = document.getElementById(id); if (el) el.value = '';
    });
    document.getElementById('search-results-wrap').style.display = 'none';
    document.getElementById('search-stats').style.display = 'none';
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
    ...
    <div class="card">
      ...
      <div style="font-size:13px;line-height:2.2;color:var(--muted)">
        ...
        <div>IA modelo: <?=htmlspecialchars($cfg['ai']['model']??'—')?></div>
    ...
    <div class="card">
      ...
      <div style="font-size:13px;color:var(--muted);line-height:2">
        <div>Provider: <strong style="color:var(--text)"><?=htmlspecialchars($cfg['ai']['provider']??'—')?></strong></div>
        <div>Modelo: <strong style="color:var(--text)"><?=htmlspecialchars($cfg['ai']['model']??'—')?></strong></div>
        <div>API Key: <strong style="color:var(--text)"><?=($cfg['ai']['api_key']??'')?'sk-ant-...'.substr($cfg['ai']['api_key'],-6):'No configurada'?></strong></div>
        <div>Estado: <?=($cfg['ai']['enabled']??false)?'<span class="chip chip-green">Activa</span>':'<span class="chip chip-red">Inactiva</span>'?></div>
      </div>
    </div>
