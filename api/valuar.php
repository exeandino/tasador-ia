...
$escritura_report     = is_array($input['escritura_report'] ?? null) ? $input['escritura_report'] : null;
$escritura_tipo       = strtolower(trim($input['escritura_tipo']   ?? 'escritura'));
$construction_quality = strtolower(trim($input['construction_quality'] ?? 'estandar'));
$valid_qualities      = ['economica','estandar','calidad','premium'];
if (!in_array($construction_quality, $valid_qualities)) $construction_quality = 'estandar';
$construction_only    = !empty($input['_construction_only']);
...
// ── Shortcut: solo costo de construcción (desde selector de calidad) ──────────
if ($construction_only) {
    $costRow = null;
    if ($pdo) {
        foreach ([[$city, $zone_key], [$city, ''], ['', '']] as [$tryCity, $tryZone]) {
            $st = $pdo->prepare("SELECT cost_usd_m2 FROM construction_zone_costs WHERE city=? AND zone=? AND quality=? LIMIT 1");
            $st->execute([$tryCity, $tryZone, $construction_quality]);
            $costRow = $st->fetchColumn();
            if ($costRow !== false) break;
        }
    }
    $baseM2    = $costRow !== false ? (float)$costRow : 950.0;
    $uncovArea = max(0, $total_area - $covered_area);
    $total     = round($covered_area * $baseM2 + $uncovArea * $baseM2 * 0.25, 0);
    outputJson([
        'success'      => true,
        'construction' => [
            'cost_usd'     => $total,
            'cost_usd_min' => round($total * 0.88, 0),
            'cost_usd_max' => round($total * 1.12, 0),
            'cost_ars'     => round($total * $usd_rate, 0),
            'cost_per_m2'  => round($baseM2, 0),
...
// ── Precio base por m² (blend configurable: por defecto 30% portales / 70% zonas) ───
// market.correction_factor: descuento sobre precios publicados (portales suelen estar inflados)
// market.blend_weight:      peso de datos reales en el blend (0.0 = solo config, 1.0 = solo mercado)
$marketCorrectionFactor = (float)($cfg['market']['correction_factor'] ?? 0.80);
$marketBlendWeight      = (float)($cfg['market']['blend_weight']      ?? 0.30);
$configBlendWeight      = 1.0 - $marketBlendWeight;

if ($market_data['used']) {
    // Aplicar factor de corrección a precios de portales antes del blend
    $mAvg = $market_data['avg_ppm2'] * $marketCorrectionFactor;
    $mMin = $market_data['min_ppm2'] * $marketCorrectionFactor;
    $mMax = $market_data['max_ppm2'] * $marketCorrectionFactor;

    $avg_base = ($mAvg * $marketBlendWeight) + ($base_prices['avg'] * $configBlendWeight);
    $min_base = ($mMin * $marketBlendWeight) + ($base_prices['min'] * $configBlendWeight);
    $max_base = ($mMax * $marketBlendWeight) + ($base_prices['max'] * $configBlendWeight);
} else {
...
// Factor legal — basado en irregularidades detectadas en escritura/compraventa por IA
function legalDocFactor(?array $report): array {
    if (!$report || empty($report['irregularidades'])) return [1.0, ''];
    $irrs  = $report['irregularidades'];
    $alta  = 0; $media = 0; $baja = 0;
    foreach ($irrs as $i) {
        $sev = strtolower($i['severidad'] ?? 'baja');
        if ($sev === 'alta')       $alta++;
        elseif ($sev === 'media')  $media++;
        else                       $baja++;
    }
    $grav       = $report['gravamenes_y_cargas'] ?? [];
    $hipotecas  = is_array($grav['hipotecas'] ?? null) ? $grav['hipotecas'] : [];
    $embargos   = is_array($grav['embargos']  ?? null) ? $grav['embargos']  : [];
    $penalty    = 0.0;
    $penalty   += $alta  * 0.04;   // -4% por irregularidad alta
    $penalty   += $media * 0.015;  // -1.5% por irregularidad media
    $penalty   += $baja  * 0.005;  // -0.5% por irregularidad baja
    if (count($hipotecas) > 0) $penalty += 0.03;  // -3% por hipoteca activa
    if (count($embargos)  > 0) $penalty += 0.05;  // -5% por embargo activo
    $penalty    = min($penalty, 0.15);             // máximo -15%
    $factor     = round(1.0 - $penalty, 4);
    $irCount    = count($irrs);
    $label      = $penalty > 0
        ? sprintf('%d irreg. doc. (-%s%%)', $irCount, number_format($penalty * 100, 1, '.', ''))
        : 'Documento sin observaciones';
    return [$factor, $label];
}
[$legal_f, $legal_l] = legalDocFactor($escritura_report);
...
if ($escritura_report && $legal_l) {
    $multipliers['Análisis doc. legal'] = ['factor' => $legal_f, 'label' => $legal_l];
}
...
outputJson([
    ...
    'market_data'  => array_merge($market_data, [
        'correction_factor' => $marketCorrectionFactor,
        'blend_weight'      => $marketBlendWeight,
    ]),
    ...
    'construction' => (function() use ($city, $zone_key, $covered_area, $total_area, $prop_type, $age_years, $price_avg, $pdo, $usd_rate, $construction_quality): ?array {
        // Calcula el costo de construcción inline para incluirlo en la respuesta
        // Solo para propiedades con área > 0 (no terrenos)
        if ($covered_area <= 0 || $prop_type === 'terreno') return null;
        try {
            // Consulta inline del costo base (misma lógica que construction.php)
            $quality = $construction_quality;
            $costRow = null;
            if ($pdo) {
                foreach ([[$city, $zone_key], [$city, ''], ['', '']] as [$tryCity, $tryZone]) {
            ...
            $baseM2 = $costRow !== false ? (float)$costRow : 980.0;
            $uncovArea  = max(0, $total_area - $covered_area);
            $total      = round($covered_area * $baseM2 + $uncovArea * $baseM2 * 0.25, 0);
            $totalMin   = round($total * 0.88, 0);
            $totalMax   = round($total * 1.12, 0);
            // Incidencia del terreno
            $landEst    = max(0, $price_avg - $total);
            $incidencia = $price_avg > 0 ? round(($landEst / $price_avg) * 100, 1) : null;
            return [
                'cost_usd'     => $total,
                'cost_usd_min' => $totalMin,
                'cost_usd_max' => $totalMax,
                'cost_ars'     => round($total * $usd_rate, 0),
                'cost_per_m2'  => round($baseM2, 0),
                'quality'      => $quality,
                'land_est_usd' => $landEst,
