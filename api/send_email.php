...
function buildHtml(array $p): string {
    ...
    // ── Análisis de escritura / compraventa ──
    $escrituraHtml = '';
    $escReport = $p['escritura_report'] ?? null;
    $escTipo   = (string)($p['escritura_tipo'] ?? 'escritura');
    if (!empty($escReport) && is_array($escReport)) {
        $eTitle = $escTipo === 'compraventa' ? '📋 Boleto de Compraventa' : '📜 Escritura Pública';
        $eRows  = '';

        // Helper para fila de dato
        $eRow = function(string $label, $val) use ($bg) {
            if ($val === null || $val === '' || $val === false) return '';
            $v = is_array($val) ? implode(', ', array_map('strval', $val)) : (string)$val;
            return "<tr><td style='padding:5px 12px;color:#888;font-size:11px;border-bottom:1px solid #eee;white-space:nowrap;width:130px'>$label</td>"
                 . "<td style='padding:5px 12px;font-size:12px;font-weight:600;border-bottom:1px solid #eee'>" . htmlspecialchars($v) . "</td></tr>";
        };

        if ($escTipo === 'compraventa') {
            $im   = $escReport['inmueble']              ?? [];
            $par  = $escReport['partes']                ?? [];
            $cond = $escReport['condiciones_economicas'] ?? [];
            $gar  = $escReport['garantias_y_condiciones'] ?? [];
            $not  = $escReport['datos_notariales']      ?? [];
            $irrs = $escReport['irregularidades']       ?? [];
            $eRows .= $eRow('Dirección',      $im['direccion_completa'] ?? null);
            $eRows .= $eRow('Superficie',     $im['superficie_total']   ?? null);
            $eRows .= $eRow('Matrícula',      $im['matricula']           ?? null);
            if (!empty($par['vendedor']['nombre']))  $eRows .= $eRow('Vendedor',   $par['vendedor']['nombre']  . (!empty($par['vendedor']['dni_cuit']) ? ' · DNI/CUIT: '.$par['vendedor']['dni_cuit'] : ''));
            if (!empty($par['comprador']['nombre'])) $eRows .= $eRow('Comprador',  $par['comprador']['nombre'] . (!empty($par['comprador']['dni_cuit']) ? ' · DNI/CUIT: '.$par['comprador']['dni_cuit'] : ''));
            $eRows .= $eRow('Precio total',   $cond['precio_total']     ?? null);
            $eRows .= $eRow('Moneda',         $cond['moneda']            ?? null);
            $eRows .= $eRow('Seña',           $cond['seña']              ?? null);
            $eRows .= $eRow('Forma de pago',  $cond['forma_de_pago']    ?? null);
            $eRows .= $eRow('Escrituración',  $cond['fecha_escrituracion'] ?? null);
            $eRows .= $eRow('Posesión',       $cond['posesion']          ?? null);
            $eRows .= $eRow('Escribano',      $not['escribano']          ?? null);
            $eRows .= $eRow('Fecha',          $not['fecha']              ?? null);
            if (!empty($escReport['resumen_legal'])) {
                $eRows .= "<tr><td colspan='2' style='padding:8px 12px;font-size:12px;color:#444;font-style:italic;border-bottom:1px solid #eee'>"
                        . htmlspecialchars($escReport['resumen_legal']) . "</td></tr>";
            }
        } else {
            $im  = $escReport['inmueble']         ?? [];
            $med = $im['medidas_terreno']          ?? [];
            $lim = $im['limites']                  ?? [];
            $not = $escReport['datos_notariales']  ?? [];
            $grav= $escReport['gravamenes_y_cargas'] ?? [];
            $irrs= $escReport['irregularidades']   ?? [];
            $tits= $escReport['titulares_actuales'] ?? [];
            $hist= $escReport['historial_transmisiones'] ?? [];
            $eRows .= $eRow('Dirección',      $im['direccion_completa']  ?? null);
            $eRows .= $eRow('Sup. total',     $im['superficie_total']    ?? null);
            $eRows .= $eRow('Sup. cubierta',  $im['superficie_cubierta'] ?? null);
            $eRows .= $eRow('Matrícula',      $im['matricula']            ?? null);
            $eRows .= $eRow('Catastro',       $im['nomenclatura_catastral'] ?? null);
            if ($med['frente'] ?? null)    $eRows .= $eRow('Frente',  $med['frente']);
            if ($med['fondo'] ?? null)     $eRows .= $eRow('Fondo',   $med['fondo']);
            if ($lim['norte'] ?? null)     $eRows .= $eRow('Límite N', $lim['norte']);
            if ($lim['sur']   ?? null)     $eRows .= $eRow('Límite S', $lim['sur']);
            foreach ($tits as $t) {
                if (!empty($t['nombre'])) {
                    $ti = $t['nombre'];
                    if (!empty($t['dni']))          $ti .= ' · DNI '.$t['dni'];
                    if (!empty($t['estado_civil'])) $ti .= ' · '.$t['estado_civil'];
                    if (!empty($t['porcentaje']))   $ti .= ' ('.$t['porcentaje'].')';
                    $eRows .= $eRow('Titular', $ti);
                }
            }
            // Historial comprimido
            if (!empty($hist)) {
                $hStr = implode(' → ', array_map(fn($h) => ($h['adquirente'] ?? '?').($h['fecha'] ? ' ('.$h['fecha'].')' : ''), $hist));
                $eRows .= $eRow('Historial', $hStr);
            }
            $eRows .= $eRow('Escribano',    $not['escribano']    ?? null);
            $eRows .= $eRow('Fecha',        $not['fecha']        ?? null);
            $eRows .= $eRow('Precio decl.', $not['precio_declarado'] ?? null);
            // Hipotecas
            if (!empty($grav['hipotecas'])) {
                foreach ($grav['hipotecas'] as $hip) {
                    $hStr = ($hip['acreedor'] ?? '').($hip['monto'] ? ' · '.$hip['monto'] : '').($hip['vencimiento'] ? ' · vto '.$hip['vencimiento'] : '');
                    $eRows .= $eRow('Hipoteca', $hStr);
                }
            }
            if (!empty($escReport['resumen_ejecutivo'])) {
                $eRows .= "<tr><td colspan='2' style='padding:8px 12px;font-size:12px;color:#444;font-style:italic;border-bottom:1px solid #eee'>"
                        . htmlspecialchars($escReport['resumen_ejecutivo']) . "</td></tr>";
            }
        }
        ...
        $irrHtml = '';
        if (!empty($irrs)) {
            $sevColors = ['alta' => '#dc3545', 'media' => '#e6ac00', 'baja' => '#6c757d'];
            foreach ($irrs as $irr) {
                $sev   = strtolower($irr['severidad'] ?? 'baja');
                $col   = $sevColors[$sev] ?? '#6c757d';
                $tipo  = htmlspecialchars($irr['tipo']        ?? 'Irregularidad');
                $desc  = htmlspecialchars($irr['descripcion'] ?? '');
                $rec   = htmlspecialchars($irr['recomendacion'] ?? '');
                $irrHtml .= "<tr><td colspan='2' style='padding:8px 12px;border-bottom:1px solid #eee;border-left:3px solid $col'>
                    <span style='background:$col;color:#fff;border-radius:3px;padding:1px 6px;font-size:10px;font-weight:700;text-transform:uppercase'>$sev</span>
                    <strong style='font-size:12px;margin-left:6px'>$tipo</strong><br>
                    <span style='font-size:11px;color:#666'>$desc</span>"
                    . ($rec ? "<br><span style='font-size:11px;color:#888'>Recomendación: $rec</span>" : '')
                    . "</td></tr>";
            }
        }

        $escrituraHtml = "<tr><td style='padding:14px 32px 0'>
          <h3 style='margin:0 0 10px;font-size:11px;text-transform:uppercase;letter-spacing:.5px;color:$dark'>$eTitle</h3>
...
try {
    ...
    $params['escritura_report'] = $prop['escritura_report'] ?? null;
    $params['escritura_tipo']   = $prop['escritura_tipo']   ?? 'escritura';
    ...
    try {
        ...
        // ── Guardar escritura analizada ───────────────────────────────────────
        $escReport = $prop['escritura_report'] ?? null;
        if (!empty($escReport) && is_array($escReport)) {
            $escTipoDb   = (string)($prop['escritura_tipo'] ?? 'escritura');
            $im          = $escReport['inmueble'] ?? [];
            $direc       = (string)($im['direccion_completa'] ?? '');
            $matricula   = (string)($im['matricula'] ?? '');
            $superficie  = (string)($im['superficie_total'] ?? '');
            $irrsDb      = $escReport['irregularidades'] ?? [];
            $irrCount    = count($irrsDb);
            $irrAlta     = count(array_filter($irrsDb, fn($i) => strtolower($i['severidad'] ?? '') === 'alta'));
            // Titular(es) para escritura, partes para compraventa
            $titularStr  = '';
            if ($escTipoDb === 'compraventa') {
                $partes = $escReport['partes'] ?? [];
                $titularStr = implode(' / ', array_filter([
                    $partes['vendedor']['nombre']  ?? null,
                    $partes['comprador']['nombre'] ?? null,
                ]));
                $precioDecl = (string)($escReport['condiciones_economicas']['precio_total'] ?? '');
            } else {
                $tits = $escReport['titulares_actuales'] ?? [];
                $titularStr = implode(', ', array_filter(array_map(fn($t) => $t['nombre'] ?? null, $tits)));
                $precioDecl = (string)($escReport['datos_notariales']['precio_declarado'] ?? '');
            }
            try {
                $pdo->exec("CREATE TABLE IF NOT EXISTS escrituras_store (
                    id              INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
                    tasacion_code   VARCHAR(20),
                    tipo            VARCHAR(20) DEFAULT 'escritura',
                    direccion       VARCHAR(300),
                    matricula       VARCHAR(100),
                    superficie      VARCHAR(60),
                    titulares       VARCHAR(400),
                    precio_declarado VARCHAR(80),
                    irregularidades_count TINYINT UNSIGNED DEFAULT 0,
                    irregularidades_alta  TINYINT UNSIGNED DEFAULT 0,
                ...
                $pdo->prepare("INSERT INTO escrituras_store
                    (tasacion_code,tipo,direccion,matricula,superficie,titulares,precio_declarado,
                     irregularidades_count,irregularidades_alta,contact_name,contact_email,full_report)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)")
                    ->execute([$code,$escTipoDb,$direc,$matricula,$superficie,$titularStr,$precioDecl,
                               $irrCount,$irrAlta,$fullName,$email,json_encode($escReport, JSON_UNESCAPED_UNICODE)]);
            } catch (\Throwable $ignored) {}
        }
    } catch (\Throwable $ignored) {}

    // ── Auto-publicar en WordPress/Houzez ──────────────────────────────────────
    $wpResult = null;
    $wpCfg    = $cfg['wordpress'] ?? [];
    if (!empty($wpCfg['enabled']) && !empty($wpCfg['url']) && !empty($wpCfg['user']) && !empty($wpCfg['app_pass'])) {
        require_once __DIR__ . '/wp_publish.php';
        $wpData = [
            'property_type'   => (string)($prop['property_type']   ?? ''),
            'operation'       => (string)($prop['operation']        ?? 'venta'),
            'covered_area'    => (float)($prop['covered_area']      ?? 0),
            'uncovered_area'  => (float)($prop['uncovered_area']    ?? 0),
            'bedrooms'        => (int)($prop['bedrooms']            ?? 0),
            'bathrooms'       => (int)($prop['bathrooms']           ?? 0),
            'garages'         => (int)($prop['garages']             ?? 0),
            'age_years'       => (int)($prop['age_years']           ?? 0),
            ...
            'zone'            => (string)($result['zone']['zone']   ?? ''),
            'city'            => (string)($result['zone']['city']   ?? ''),
            'price_suggested' => (float)($result['price']['suggested'] ?? 0),
            'price_min'       => (float)($result['price']['min']    ?? 0),
            'price_max'       => (float)($result['price']['max']    ?? 0),
            'price_ars'       => (float)($result['price_ars']['suggested'] ?? 0),
            'code'            => $code,
            'contact_name'    => $fullName,
            'contact_email'   => $email,
            'contact_phone'   => $phone,
        ];
        $wpResult = createWpProperty($wpData, $wpCfg);
    }
